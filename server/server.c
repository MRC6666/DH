...
void exchange_dh_key(int sockfd, mpz_t s)
{
    ...
    // mpz_set_ui(server_dh_key.g, (unsigned long int)5); // g
    // 从客户端接收p
    bzero(buf, MAX);
    printf("等待从客户端接收p...\n\n");
    ...
    // mpz_init_set_str(server_dh_key.pri_key, "12345678954671", 10); //b=5
...
int psk(int sockfd)
{
    ...
    getchar();
    memcpy(ch, "msg", 3);
    write(sockfd, ch + 3, sizeof(ch)); // 明文发送给客户端
    bzero(text, 33);
    read(sockfd, text, sizeof(text));
    printf("客户端返回的psk密文为：");
    for (int i = 0; i < 32; ++i)
    ...
    printf("解密得到的明文为: %s\n\n", text + 3);
    // 比较前后字符串是否相同
    flag = strncmp(ch + 3, text, PSK_LEN);
